...
1. **核心要求**：
    ...
    - 然後按照從上到下、從左到右的順序讀取字，形成新的字串

2. **關鍵特性**：
    - 字的排列遵循先向下移動 `numRows - 1` 步，然後向上移動 `numRows - 1` 步的模式
    - 當 `numRows = 1` 時，沒有曲折，直接返回原字串
    - 當 `numRows >= s.length` 時，所有字都會在第一列（但仍然需要按照規則排列）

3. **邊界情況**：
    - 空字串：根據約束條件，不會出現
    - 單字字串：直接返回原字串
    - `numRows = 1`：沒有曲折，直接返回原字串
    - `numRows >= s.length`：所有字都會在同一列（但排列仍然需要遵循規則）
...
理解這個問題的關鍵是認識到，這裡的「曲折」不是真正的「Z」形，而是一種垂直的「之」字形路徑，其中字按照固定的模式先向下，再向上移動。
...
1. 字的排列遵循先向下，再向上的模式
2. 字"P"在第一行，然後向下到"A"（第二行），再向下到"Y"（第三行）
...
最直觀的方法是完全模擬曲折過程，先創建一個二維矩陣來存儲字的位置，然後按照曲折的路徑填充字。最後，按從上到下、從左到右的順序讀取非空字，形成結果字串。
...
觀察曲折模式後，可以發現一個數學規律：對於給定的行數 `numRows`，曲折模式的週期是 `2 * (numRows - 1)`。可以根據這個週期，直接計算每個字應該位於哪一行。

然後，可以為每一行創建一個字串，按照字的順序將其分配到對應的行，最後將所有行的字串連接起來。
...
也可以反過來思考，直接計算每一行包含哪些原始字串中的字。根據曲折模式，可以推導出每一行字的索引模式。
...
3. 遍歷原始字串的每個字，根據曲折模式的數學關係，決定將其添加到哪一行
...
2. 在一個週期內，字的行索引先從 0 增加到 n-1，然後再從 n-2 減少到 1

基於這個規律，可以計算原始字串中每個索引 `i` 的字應該被放在哪一行。
...
這種方法直接計算每個字應該在哪一行，不需要模擬整個曲折路徑，效率較高。
...
1. 遍歷原始字串的每個字：O(n)，其中 n 是字串的長度
2. 計算每個字的行索引：O(1)
3. 將字添加到對應行的字串：平均情況下是 O(1)
4. 連接所有行的字串：O(n)
...
1. **預分配空間**：可以預先計算每一行的字數量，然後為每一行的字串分配足夠的空間，減少字串動態擴容的次數。
