...
#### 方案 1：左到右掃描（基本方法）
對字串從左到右掃描，檢查每個字：
- 如果當前字表示的值大於等於下一個字的值，則加上當前字的值
- 如果當前字的值小於下一個字的值，則減去當前字的值（處理減法規則）
...
#### 方案 3：預處理減法情況
預先處理所有可能的二字減法組合（IV, IX, XL, XC, CD, CM）：
- 先檢查當前位置是否以這些二字減法模式開始
- 如果是，加上相應的值，並跳過這兩個字
- 否則，只加上當前字的值
...
### 空間複雜度：
- 我們使用了一個固定大小的 map 來存儲羅馬數字到整數的映射，這需要 O(1) 的額外空間。
- 除此之外，我們只使用了幾個變數，所以總空間複雜度為 O(1)。

### 無法進一步優化的理由：
- 時間複雜度已經是 O(n)，這是必須的，因為我們至少需要讀取每個字一次。
- 空間複雜度為 O(1)，已經是最優的。
