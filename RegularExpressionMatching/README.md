...
## 繁體中文版本
給你一個字串 `s` 和一個模式 `p`，實現支持 '.' 和 '*' 的正則表達式匹配：
- '.' 匹配任意單個字。
- '*' 匹配零個或多個前面的元素。
...
- s 僅包含小寫英文字母。
- p 僅包含小寫英文字母、'.' 和 '*'。
- 對於字 '*' 的每次出現，前面都保證有一個有效的字匹配。

# 2. 問題理解

## 核心要求與約束
這個問題要求我們實現一個支持兩種特殊字的正則表達式匹配器：
- '.' 可以匹配任何單一字
- '*' 可以匹配零個或多個前面的元素

匹配必須覆蓋整個輸入字串，而不僅僅是部分匹配。
...
## 方法一：遞歸法（帶備忘錄）
最直觀的方法是使用遞歸。我們可以按字逐一比較，並針對特殊字 '.' 和 '*' 做特殊處理：
- 如果當前字匹配或模式是 '.'，則繼續匹配下一個字
- 如果模式中有 '*'，我們需要考慮以下情況：
    - 匹配零次前面的字（跳過當前模式字和 '*'）
    - 匹配一次或多次前面的字（如果當前字匹配）
...
## 方法二：動態規劃法
我們可以定義一個二維數組 `dp[i][j]`，表示字串 `s` 的前 `i` 個字是否能匹配模式 `p` 的前 `j` 個字。

基本情況：
- `dp[0][0] = true`（空字串匹配空模式）
- `dp[i][0] = false` 對於 `i > 0`（空模式不能匹配非空字串）
- `dp[0][j]` 需要根據模式而定（例如，模式如 "a*b*c*" 可以匹配空字串）
...
## 方法三：非確定性有限自動機（NFA）
我們可以將正則表達式轉換為 NFA，然後模擬 NFA 在輸入字串上的執行。這種方法適用於更複雜的正則表達式，但對於這個問題來說可能過於複雜。
...
## 識別相似模式
這類問題屬於字串匹配與動態規劃的結合。當遇到需要處理複雜匹配規則的問題時，我們應該考慮使用動態規劃，尤其是當問題具有重疊子問題和最優子結構時。
...
## 初步思路
首先，我們需要定義狀態：`dp[i][j]` 表示字串 `s` 的前 `i` 個字是否能匹配模式 `p` 的前 `j` 個字。
...
### 動態規劃解法:
- 我們需要填充大小為 (m+1) × (n+1) 的 dp 數組，其中 m 是字串 s 的長度，n 是模式 p 的長度
...
1. 它直觀且容易理解
2. 時間複雜度已是最優 O(m × n)
3. 在實際情況下，字串和模式的長度通常不會太長，所以空間消耗並不是瓶頸

## 相關問題推薦
1. LeetCode 44: 通配符匹配 (Wildcard Matching)
2. LeetCode 72: 編輯距離 (Edit Distance)
3. LeetCode 97: 交錯字串 (Interleaving String)
