...
### 繁體中文版：
給定一個只包含 '(' 和 ')' 的字串，找出最長有效（格式正確）括號子串的長度。
...
這個問題要求我們找出給定字串中最長的有效括號子串長度。有效括號意味著每個開括號 '(' 都有一個對應的閉括號 ')'，且它們按照正確的順序配對。
...
### 方法二：動態規劃
我們可以定義 dp[i] 為以第 i 個字結尾的最長有效括號子串的長度。
