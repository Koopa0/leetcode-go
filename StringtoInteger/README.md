...
實現 `myAtoi(string s)` 函數，將字串轉換為 32 位有符號整數（類似於 C/C++ 中的 `atoi` 函數）。

`myAtoi(string s)` 的算法如下：
1. 讀取並忽略任何前導空格。
2. 檢查下一個字（如果尚未到字串的末尾）是 '-' 還是 '+'。如果存在，讀取此字。這決定了最終結果是負數還是正數。如果兩者都不存在，則假設結果為正數。
3. 讀取下一個字，直到遇到下一個非數字字或輸入的末尾。字串的其餘部分將被忽略。
...
**核心需求與約束：**
- 需要將一個字串轉換為一個 32 位有符號整數
- 需要處理各種邊緣情況，例如空白字、正負號、非數字字等
- 結果必須被限制在 32 位有符號整數的範圍內：[-2^31, 2^31 - 1]

**輸入/輸出特性：**
- 輸入：一個由字母、數字、空格和特殊字組成的字串
- 輸出：一個 32 位有符號整數
...
4. 遇到非數字字時停止讀取
5. 檢查溢出情況並進行限制
6. 處理各種邊緣情況，例如空字串、只有空白字的字串、只有符號沒有數字的字串等
...
這是一個直觀的方法，它逐字處理輸入字串，按照問題描述的步驟執行。

### 方法 2：使用正則表達式

使用正則表達式來匹配符合條件的字串部分，然後將其轉換為整數。
...
步驟 2: 處理符號
  "-42" -> 識別到 '-'，設置 sign = -1 (表示負數)，並移動到下一個字
...
- **最佳情況**：O(1)，當輸入是空字串或只有空白字時
- **平均情況**：O(n)，其中 n 是輸入字串的長度，因為在最壞的情況下需要遍歷整個字串
- **最壞情況**：O(n)，當整個字串都是數字時
...
時間複雜度：
- 跳過前導空白：最壞情況下需要遍歷整個字串，O(n)
- 處理符號：O(1)，只需檢查一個字
- 讀取數字並轉換：最壞情況下需要遍歷整個字串，O(n)
- 檢查溢出並返回結果：O(1)
...
1. **早期返回**：如果在跳過前導空白後發現字串為空或第一個非空白字既不是數字也不是符號，可以直接返回 0。
